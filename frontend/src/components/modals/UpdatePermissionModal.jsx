...
const UpdatePermissionModal = ({ onSubmitSuccess }) => {
  ...
  const [updatePermission, { loading: mutationLoading, error }] = useMutation(UpdatePermission, {
    refetchQueries: [{ query: GetAllPermission }]
  });

  const permissionOptions = data?.getAllPermission?.map(p => ({
    value: p.id,
  ...
  const formConfig = [
    {
      ...
      type: 'select',
      placeholder: queryLoading ? 'Loading permissions...' : 'Choose a permission',
