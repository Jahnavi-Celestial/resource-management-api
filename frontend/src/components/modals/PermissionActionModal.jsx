  ...
  const formSchema = [
    {
      ...
      validators: [{ type: "required", message: "Please select a target role" }],
      options: (rolesData?.getAllRoles || []).map(role => ({ value: role.id, label: role.role_name }))
    },
    {
      name: "permissionIds",
      label: "Select System Permissions",
      defaultValue: [],
      validators: [{ type: "required", message: "Please select at least one permission" }],
      renderCustom: ({ name, value: selectedList, onChange, externalError }) => {
        ...
        return (
          <div key={name} className="form-group">
            ...
            <div className="list-container" style={{ maxHeight: "200px", overflowY: "auto", padding: "10px" }}>
              {(permissionsData?.getAllPermission || []).map((perm) => {
                const permId = Number(perm.id)
                return (
                  <div key={permId} className="flex-center" style={{ margin: "6px 0" }}>
                    <input
                      type="checkbox"
                      id={`perm-${permId}`}
                    ...
                    />
                    <label htmlFor={`perm-${permId}`} style={{ cursor: "pointer" }}>{perm.permission_name}</label>
                  </div>
                )
              })}
            </div>
    ...
    <div className="form-container">
      <div className="form-card">
        <div className="form-header">
          <h1 className="form-title">{actionType === "assign" ? "Grant Permissions" : "Revoke Permissions"}</h1>
