...
import { DeleteRole, DeletePermission } from '../../graphql/mutations';

const DeleteRoleOrPermissionModal = ({ type, onSubmitSuccess }) => {
  const isRole = type === 'role';
  
  const { data: roleData, loading: roleLoading } = useQuery(GetAllRoles, { skip: !isRole });
  const { data: permData, loading: permLoading } = useQuery(GetAllPermission, { skip: isRole });
  
  const [deleteRole, { loading: delRoleLoading }] = useMutation(DeleteRole, { refetchQueries: [{ query: GetAllRoles }] });
  const [deletePermission, { loading: delPermLoading }] = useMutation(DeletePermission, { refetchQueries: [{ query: GetAllPermission }] });

  const rawOptions = isRole ? roleData?.getAllRoles : permData?.getAllPermission;
