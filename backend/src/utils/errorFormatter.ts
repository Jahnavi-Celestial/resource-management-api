...
export const formatError = (formattedError: GraphQLFormattedError, error: any): GraphQLFormattedError => {
  ...
  if (formattedError.extensions?.validationErrors) {
    ...
    return {
      message: error?.constraints?.customMin || error?.constraints?.customMinLength || error?.constraints?.customIsEmail || error?.constraints?.customIsAlpha || formattedError.message,
      extensions: {
